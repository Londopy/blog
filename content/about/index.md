---
title: "About"
description: "Notes on security, systems, radio, and building things, plus whatever else I'm chewing on."
hidemeta: true
hiddenInRss: true  # keep the feed (and dev.to's import of it) posts-only
ShowToc: false
ShowPostNavLinks: false
---

I'm Londopy. I'm studying engineering with a focus on field work, and heading toward OT security: the computers that run power grids, water plants, and factories. Most of what I build starts as a question I can't stop turning over.

Away from the keyboard, I surf, climb, and freedive. I've been pescatarian for more than eight years, and Daoist philosophy shapes how I think about most things.

{{< figure src="climbing-headlamp.jpg" alt="A climbing shoe on the rock and a trad rack of cams and nuts, lit by a headlamp at night, with a purple rope running up the wall" caption="Climbing by headlamp." >}}

## What I write about

Mostly security and systems: git internals, tooling, and the industrial control systems I'm heading toward. Some radio. And now and then, whatever I've been reading or eating, like the history of which animals religions have counted as fish.

## How this blog works

New posts go up on Thursdays, and you can [get each one by email or RSS]({{< relref "subscribe" >}}). Everything publishes here first, and copies on other sites link back to the original. If you spot a mistake, tell me through the [contact page]({{< relref "contact" >}}) and I'll fix it.

## Elsewhere

- Projects: [londopy.github.io](https://londopy.github.io/)
- Code: [github.com/Londopy](https://github.com/Londopy)
- Feed: [RSS](https://londopy.github.io/blog/index.xml)
- API: [every post as JSON]({{< relref "api" >}})
- Contact: [message form, IRC, Discord]({{< relref "contact" >}})

## License

Writing on this site is licensed [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Code snippets are [MIT](https://github.com/Londopy/blog/blob/main/LICENSE). The source for the whole site is at [Londopy/blog](https://github.com/Londopy/blog).
